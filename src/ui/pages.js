 ...
 *
 * CHANGES:
 *  ✅ dashboardHTML() — added "📐 Set Geofence" button directly in right panel
 *  ✅ settingsHTML()  — added Notifications section (push + sound toggles)
 *  ✅ appShellHTML()  — notification permission banner slot
 */
  ...
  <div class="dash-split">
    ...
    <div class="map-wrap">
      ...
      <div class="map-ctrls">
        ...
        <button class="map-btn" id="btn-clear-route" style="display:none;color:var(--danger)">✕ Clear Route</button>
      </div>

      <!-- Map search (top-right) -->
      <div class="map-search-wrap">
        <input type="text" id="map-search-input" class="map-search-input"
          placeholder="🔍 Search any location…" autocomplete="off" spellcheck="false"
          aria-label="Search location" aria-autocomplete="list" aria-expanded="false"/>
    ...
    <div class="right-panel">
      ...
      <div class="panel-sec">
        ...
        <div class="dev-status-card">
          ...
          <!-- ✅ GEOFENCE BUTTON — directly in panel -->
          <button class="panel-gf-btn" id="panel-geofence-btn">📐 Set Geofence</button>
        </div>
      </div>
        ...
        <div class="route-panel">
          <div class="dest-search-wrap">
            <input type="text" class="dest-input" id="dest-input"
              placeholder="Search destination…" autocomplete="off" aria-label="Destination search"/>
          ...
          <div class="route-btn-row">
            ...
            <button class="route-btn danger-btn" id="clear-route-btn" style="display:none">✕ Clear</button>
          </div>
        </div>
      </div>
      ...
      <div class="panel-sec">
        ...
        <div class="playback-wrap">
          <div class="pb-bar">
            ...
            <button class="pb-btn" id="pb-clear-btn" title="Hide Playback">✕</button>
          </div>
          <div class="pb-info">
            <span id="pb-cur">0%</span>
            <span id="pb-total">-- pts</span>
          </div>
          <div class="pb-no-data" id="pb-no-data">No route data yet — device must move to record a path.</div>
        </div>
      </div>
...
export function analyticsHTML() {
  ...
  <div class="analytics-pg">
    ...
    <div class="chart-card full">
      <div class="chart-hdr"><div><div class="chart-title">Distance Per Device</div><div class="chart-sub">Total km traveled this session</div></div></div>
...
export function alertsHTML() {
  ...
  const icons = { offline:'⚫', geofence:'🔵', speed:'🟠', crash:'🔴', sync:'🟢' };
  ...
  <div class="alerts-pg">
    <div class="alert-toolbar">
      ...
      <div class="alert-actions">
        <button class="alert-action-btn" id="btn-mark-read">✓ Mark Read</button>
        <button class="alert-action-btn danger" id="btn-clear-alerts">🗑 Clear All</button>
      </div>
    </div>
...
export function settingsHTML() {
  ...
  <div class="settings-pg">
    ...
    <div class="settings-sec">
      ...
      <div class="gf-note">
        ⚠️ To set a geofence: select a device, then click <b>📐 Set Geofence</b> in the right panel
        or in the map controls — then click anywhere on the map to place the centre.
        Press <b>Esc</b> to cancel.
      </div>
      <div style="padding:12px 16px 0">
        <table class="gf-device-table">
          <thead><tr><th>Device</th><th>Lat</th><th>Lng</th><th>Radius (m)</th><th>Active</th></tr></thead>
      ...
      <div style="padding:12px 16px 16px">
        <button class="btn-save" id="btn-save-geofences" style="font-size:13px;padding:9px 24px">💾 Save Geofences</button>
      </div>
    </div>
    ...
    <div class="settings-sec">
      ...
      <div class="settings-row">
        ...
        <input type="number" class="settings-input" id="s-offline-t" value="90" min="5" max="300"/>
      </div>
      <div class="settings-row">
        <div><div class="settings-label">Overspeed Threshold (km/h)</div><div class="settings-desc">Alert when speed exceeds this</div></div>
        <input type="number" class="settings-input" id="s-speed-t" value="120" min="10" max="300"/>
      </div>
      <div class="settings-row">
        <div><div class="settings-label">Crash Threshold (g-force)</div><div class="settings-desc">MPU6050 impact detection sensitivity</div></div>
        <input type="number" class="settings-input" id="s-crash-t" value="2.0" min="0.5" max="10" step="0.1"/>
      </div>
      <div class="settings-row">
        <div><div class="settings-label">Geofence Alert Cooldown (s)</div><div class="settings-desc">Min seconds between geofence alerts</div></div>
    ...
    <!-- Notifications -->
    <div class="settings-sec">
      <div class="settings-sec-hdr">🔔 Notifications</div>
      <div class="settings-row">
        <div><div class="settings-label">Browser Notifications</div><div class="settings-desc">OS-level push for crash, speed & offline alerts</div></div>
        <div class="toggle on" id="s-notif-toggle"></div>
      </div>
      <div class="settings-row">
        <div><div class="settings-label">Alert Sound</div><div class="settings-desc">Audio beep for critical alerts</div></div>
        <div class="toggle on" id="s-notif-sound-toggle"></div>
      </div>
    </div>
    ...
    <div class="settings-sec">
      ...
      <div class="settings-row">
        <div><div class="settings-label">Offline Mode</div><div class="settings-desc">Auto-buffer when GPS signal is lost</div></div>
      ...
      <div class="settings-row">
        <div><div class="settings-label">Max Offline Buffer</div><div class="settings-desc">Max records per device</div></div>
    ...
    <div class="settings-sec">
      ...
      <div class="settings-row">
        <div><div class="settings-label">App Version</div><div class="settings-desc">VECTOR — Real-Time Intelligent Asset Tracking</div></div>
        <span style="font-family:var(--font-mono);font-size:12px;color:var(--t3)">v3.0.0</span>
      </div>
    </div>
...
export function appShellHTML() {
  ...
  <div class="layout">
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-footer sb-profile-footer">
        <span id="last-sync" style="display:none"></span>
        <div class="sb-profile-row">
          <div class="sb-avatar" id="sb-avatar">
            <span id="sb-avatar-initials">AD</span>
          </div>
          <div class="sb-profile-info">
            <div class="sb-profile-name">Admin</div>
            <div class="sb-profile-email" id="sb-user-email">admin@vector.app</div>
          </div>
        </div>
        <div class="sb-status-pill">
          <span class="sb-status-dot" id="sb-status-dot"></span>
          <span class="sb-status-text" id="sb-status-text">Connecting…</span>
        </div>
        <button class="sb-signout-btn" id="sb-signout-btn" type="button">
          <span id="sb-signout-spinner" class="sb-signout-spinner" hidden></span>
          <span id="sb-signout-text">🚪&nbsp; Sign Out</span>
        </button>
      </div>
    </aside>
